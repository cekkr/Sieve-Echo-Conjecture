...
class Config:
    ...
    length_penalty_factor: float = 0.3 #check: wrong way?
    
    # --- DEFINITIVE DYNAMIC SAVING CONTROL ---
    # Save any algorithm found during evolution with fitness > (save_threshold_ratio * average_fitness_of_top_performers).
    save_threshold_ratio: float = 0.75
    # The percentage of top performers to average for the dynamic threshold (0.1 = top 10%).
    top_performer_percentage: float = 0.1
    # Safety Net: Guarantees that the absolute top N algorithms are always saved at the end of a cycle.
    save_top_n_at_end_of_cycle: int = 5
...
class UnifiedFormulaDiscoverer:
    ...
    def evolve_formulas(self, cycle: int = 0) -> Dict:
        """
        DEFINITIVE FIX: Implements dynamic saving threshold based on a ratio of top performers.
        """
        print("\n📊 Evolving formulas with DYNAMIC quality threshold...")
        ...
        # (Initial population generation is unchanged)
        ...
        # --- The callback is now simpler; the logic is handled by the saver ---
        def realtime_save_callback(genome, generation, rank):
            # We pass a threshold of 0 here because we want the final logic to decide
            if self.system and hasattr(self.system, 'save_algorithm_immediately'):
                 # We'll re-evaluate the threshold inside the save function based on the current population
                self.system.save_algorithm_immediately(genome, cycle, rank, self, generation, evolver.population)

        adaptive_config = { 'enabled': CONFIG.adaptive_generations, 'min_generations': CONFIG.formula_generations_min, 'stagnation_threshold': CONFIG.stagnation_threshold, 'stagnation_window': 15 }
        ...
        final_population = evolver.evolve(
            ...
            callback_top_k=5 # Check top 5 every generation for real-time saving
        )
        
        print(f"✅ Evolution complete after {evolver.generation} generations.")
        
        # --- Final "Safety Net" Saving ---
        print(f"\n💾 Applying Safety Net: Saving the absolute top {CONFIG.save_top_n_at_end_of_cycle} algorithms...")
        for rank, genome in enumerate(final_population[:CONFIG.save_top_n_at_end_of_cycle]):
            # We force-save these by passing a threshold of 0, so the fitness check always passes.
            self.system.save_algorithm_immediately(genome, cycle, rank + 1, self, evolver.generation, final_population, force_save=True)
...
class SieveEchoDiscoverySystem:
    ...
    def save_algorithm_immediately(self, genome, cycle: int, rank: int, discoverer=None, generation: int = -1, current_population: List = [], force_save: bool = False) -> bool:
        """
        DEFINITIVE FIX: Calculates and uses a dynamic fitness threshold for saving.
        
        Args:
            genome: The algorithm genome to potentially save.
            cycle, rank, discoverer, generation: Standard parameters.
            current_population: The entire sorted population from the current generation.
            force_save (bool): If True, bypasses the fitness check (used for the safety net).
        """
        try:
            # --- DYNAMIC THRESHOLD CALCULATION ---
            min_fitness_to_save = 0
            if not force_save and current_population:
                top_performers_count = int(len(current_population) * CONFIG.top_performer_percentage)
                if top_performers_count > 0:
                    top_performers = current_population[:top_performers_count]
                    # Filter out non-numeric/None fitness values
                    valid_fitness_scores = [g.fitness for g in top_performers if g.fitness is not None and np.isfinite(g.fitness)]
                    if valid_fitness_scores:
                        avg_top_fitness = np.mean(valid_fitness_scores)
                        min_fitness_to_save = avg_top_fitness * CONFIG.save_threshold_ratio
            
            # --- QUALITY GATE ---
            if not force_save and (not genome.fitness or genome.fitness < min_fitness_to_save):
                # Uncomment the line below for verbose logging of rejections
                # print(f"  - REJECTED (Fitness {genome.fitness:.4f} < Threshold {min_fitness_to_save:.4f})")
                return False

            signature = genome.get_signature()[:8]
            save_key = f"{signature}_c{cycle}"
            if save_key in self.saved_signatures:
                return False

            # (Serialization logic is unchanged)
            algorithm_data = { 'cycle': cycle, 'generation': generation, 'rank_in_gen': rank, 'fitness': genome.fitness, 'signature': genome.get_signature(), 'timestamp': datetime.now().isoformat(), 'instructions': [], 'outputs': [], 'data_config': genome.data_config }
            ...
            filename = f"cycle_{cycle:03d}_gen_{generation:04d}_rank_{rank:02d}_fit_{genome.fitness:.4f}_{signature}.json"
            filepath = self.realtime_dir / filename
            
            with open(filepath, 'w') as f:
                json.dump(algorithm_data, f, indent=2, default=str)
            
            save_type = "FORCED" if force_save else f"THRESH>{min_fitness_to_save:.3f}"
            print(f"  ✓ SAVED ({save_type}): {filename}")
