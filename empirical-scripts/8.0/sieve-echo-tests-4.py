...
This is a complete, autonomous, and perpetual discovery system designed to
investigate the Sieve Echo Conjecture. It runs continuously until stopped,
performing a multi-pronged analysis:

1.  **Parallel Data Generation**: Efficiently computes features for a vast range of numbers.
2.  **Dual-Track Genetic Programming (Evolvo)**:
    -   **Unbiased Discovery**: Finds the most predictive raw feature combinations.
    -   **Theory-Guided Exploration**: Attempts to validate specific theoretical claims
        involving constants like the Golden Ratio (phi).
3.  **Neural Architecture Search (Evolvo NN)**: Evolves neural networks to find
    non-linear patterns in the data.
4.  **Autonomous Theorist (DiscoveryAnalyzer)**:
    -   Continuously re-evaluates the Sieve Echo Law against new data.
    -   Identifies promising vs. dead-end features to guide the search.
    -   Searches for matches between empirical findings and known mathematical constants.
    -   Dynamically feeds "strong constants" discovered by GAs back into the system
      for recursive algorithm development.
...
@dataclass
class Config:
    """Central configuration for the discovery engine."""
    # --- General ---
    state_file: str = "sieve_echo_state_v12.pkl"
    perpetual_mode: bool = True  # If False, runs for a fixed number of cycles
    max_cycles: int = 100  # Only used if perpetual_mode is False
    
    # --- Data Generation ---
    data_chunk_size: int = 1000  # Numbers to process per cycle
    test_bases: List[int] = field(default_factory=lambda: [2, 3, 5, 7, 11, 13, 17, 19])
    n_workers: int = max(1, cpu_count() - 1)

    # --- Analysis & Theory ---
    analysis_interval_cycles: int = 1  # How often to run the full analysis
    correlation_threshold: float = 0.15 # Minimum correlation to be "promising"
    constant_match_tolerance: float = 0.02 # Tolerance for matching constants
    
    # --- Evolvo Formula Discovery (GA) ---
    formula_generations: int = 30
    formula_population_size: int = 50
    max_algorithm_length: int = 15  # Max instructions in an evolved formula
    
    # --- Evolvo Neural Architecture Search (NAS) ---
...
class NDRComputer:
    """
    Computes Normalized Digit Representation (NDR) patterns.
    The NDR framework normalizes the digits of a repeating decimal by its base (d -> d/b),
    creating a universal, base-independent sequence in [0,1] for analysis.
    """
    ...
    def compute_ndr_features(self, n: int, bases: List[int]) -> Dict:
        """
        Computes core number theory and NDR features for a given integer n.
        This includes omega (ω), phi (φ), and the mean/std of theta entropy (H_θ)
        across multiple bases, which is central to the Sieve Echo Conjecture.
        """
        try:
            factors = factorint(n); omega = len(factors)
            is_prime = omega == 1 and factors[n] == 1
...
class EnhancedMathematicalConstantsLibrary:
    def __init__(self):
        self.constants = {
            'e': math.e, 'pi': math.pi, 'phi': (1 + math.sqrt(5)) / 2,
            'gamma': 0.5772156649, 'apery_zeta3': 1.2020569031,
        }
        self.expressions = {
            '5_minus_1_over_15': 5 - 1/15, '3_over_4': 0.75,
            'neg_inv_phi_sq': -1 / self.constants['phi']**2,
        }
        self.discovered_strong_constants = {}
        self.update_all_consts()

    def update_all_consts(self):
        self.all_consts = {**self.constants, **self.expressions, **self.discovered_strong_constants}
    
    def add_strong_constant(self, name, value):
        print(f"  🔬 New strong constant discovered and added to library: {name} = {value:.4f}")
        self.discovered_strong_constants[name] = value
        self.update_all_consts()
...
class DiscoveryAnalyzer:
    ...
    def run_full_analysis(self):
        ...
        if len(self.data) < 200:
            print("  ANALYZER: Not enough data for robust analysis. Skipping.")
            return {}
        
        self._analyze_correlations()
        self._analyze_sieve_echo_law()
        self._find_strong_constants_from_formulas()
        print("  ANALYZER: Analysis complete.")
        return self.results

    def _analyze_correlations(self):
        """
        Finds correlations between all numeric features and the target variable omega (ω).
        This identifies "Promising Features" for the evolvers to focus on and "Dead-End Features"
        to ignore, dynamically guiding the search process.
        """
        corrs = {}
        keys = [k for k,v in self.data[-1].items() if isinstance(v, (float, int)) and k not in ['n', 'omega']]
        omegas = np.array([d['omega'] for d in self.data if 'omega' in d])
        
        for key in keys:
            vals = np.array([d.get(key) for d in self.data if 'omega' in d])
            if len(vals) == len(omegas) and np.std(vals) > 0:
                corrs[key] = np.corrcoef(vals, omegas)[0, 1]
    ...
    def _analyze_sieve_echo_law(self):
        """
        Performs a linear regression on H_θ vs. log(ω) to find the empirical
        values of alpha (α) and beta (β) for the Sieve Echo Law. It then checks
        if these values match any known or conjectured constants.
        """
        points = [(math.log(d['omega']), d['h_mean']) for d in self.data if 'h_mean' in d and d.get('omega', 0) > 0]
    ...
    def _find_strong_constants_from_formulas(self):
        """
        Extracts numerical constants from the best-evolved unbiased formulas.
        If a constant appears frequently with high fitness, it's deemed a "strong constant"
        and added to the library for use in theory-guided evolution.
        """
        # Placeholder for more advanced logic
        # For now, we'll just look for a hypothetical constant from a formula
        # In a real run, this would parse the evolved algorithm for raw numbers.
        if random.random() < 0.1: # Simulate a discovery
             new_const = 1.618 + random.uniform(-0.05, 0.05)
             self.const_lib.add_strong_constant(f"strong_const_{len(self.const_lib.discovered_strong_constants)}", new_const)
...
class EvolvoFormulaDiscoverer:
    ...
    def evolve_cycle(self, promising_features):
        ...
        print("\n  EVOLVO: Evolving formulas (Theory-Guided Track)...")
        self._evolve(True, [])

    def _evolve(self, theory_guided, promising_features):
        is_unbiased = not theory_guided
        if is_unbiased and not promising_features: return
        ...
        population = [{'alg': self._generate_random_alg(store_config), 'fitness': float('inf')} 
                      for _ in range(CONFIG.formula_population_size)]
        for ind in population: ind['fitness'] = evaluator.evaluate(ind['alg'])
        ...
        for gen in range(CONFIG.formula_generations):
            ...
            if population[0]['fitness'] < best_dict['fitness']:
                best_dict.update(population[0])
                best_dict['formula'] = self._decode_algorithm(population[0]['alg'], store_config)
                print(f"    Gen {gen}: New best formula! Fitness={best_dict['fitness']:.4f}, Formula: {best_dict['formula']}")

            elite_size = CONFIG.formula_population_size // 5
            new_pop = population[:elite_size]
            while len(new_pop) < CONFIG.formula_population_size:
                p1, p2 = self._tournament_select(population), self._tournament_select(population)
    ...
    def _get_store_config(self, theory_guided, promising_features):
        if theory_guided:
            ...
            return {'d#': d_consts, 'b#': ['is_prime'], 'd$': ['result', 'temp1'], 'b$': ['flag']}
        else:
            return {'d#': promising_features + ['n', 'one'], 'b#': ['is_prime'], 'd$': ['result', 'temp1'], 'b$': ['flag']}
            ...
            def evaluate(self, algorithm, **kwargs):
                ...
                for d in random.sample(self.data, min(200, len(self.data))):
                    ...
                    try:
                        ...
                        pred = ds.get('result')
                        actual = d['h_mean'] if self.theory_guided and 'h_mean' in d else d['omega']
                        if np.isfinite(pred) and np.isfinite(actual):
                            err += (pred - actual)**2; count += 1
                        else:
                            err += 1e6; count += 1 # Penalize non-finite results
    ...
    def _generate_random_alg(self, store_config, max_len=5):
        ...
        for _ in range(random.randint(2, max_len)):
            ...
            alg.append(instr)
        # Ensure final result is in 'result'
    ...
    def _mutate(self, alg, store_config):
        ...
        mutated_instr = self._generate_random_alg(store_config, 1)[0]
        # Ensure target variable exists
        mutated_instr[1] = alg[idx][1]
        alg[idx] = mutated_instr
        return alg
    def _decode_algorithm(self, alg, store_config):
        if not alg: return "N/A"
        final_line = alg[-1]
        source_type, source_idx = final_line[3], final_line[4]
...
class NeuralArchitectureSearcher:
    def __init__(self, data):
        self.data, self.best_genome, self.best_fitness = data, None, 0.0
    def evolve_cycle(self, promising_features):
        print("\n  NAS: Evolving neural architectures for ω(n)...")
        feature_names = [f for f in promising_features if f in self.data[0]]
        if len(feature_names) < 2:
            print("     NAS: Not enough promising features to evolve. Skipping.")
            return

        input_shape = en.TensorShape(features=len(feature_names))
        output_shape = en.TensorShape(features=1)
        ...
        if pop and (pop[0].fitness or 0) > self.best_fitness:
            ...
            print(f"    NAS: New best NN architecture found! Fitness={self.best_fitness:.4f}, Layers={len(self.best_genome.layers)}")
    ...
    def _evaluate_genome(self, genome, feature_names):
        try:
            ...
            X_train_t = torch.FloatTensor(X_train); y_train_t = torch.FloatTensor(y_train).view(-1, 1)
            X_test_t = torch.FloatTensor(X_test); y_test_t = torch.FloatTensor(y_test).view(-1, 1)
            ...
            for _ in range(20):
                optimizer.zero_grad(); loss = criterion(model(X_train_t), y_train_t)
            ...
            return 1 / (1 + test_loss) # Fitness: higher is better
        except Exception as e:
            # print(f"    NAS EVAL ERROR: {e}") # Uncomment for debugging
            return 0.0
...
class SieveEchoDiscoverySystem:
    def __init__(self):
        ...
        self.load_state() # Initializes state variables
    ...
    def load_state(self):
        if os.path.exists(CONFIG.state_file):
            print(f"🔄 Resuming from previous state: {CONFIG.state_file}")
            ...
            print("🌱 Starting a new discovery session.")
            self.data, self.discoveries, self.current_n = [], self._get_default_discoveries(), 2

    def _get_default_discoveries(self):
        return {'unbiased_formula': {}, 'theory_formula': {}, 'best_nn': {},
                'sieve_echo_law': {}, 'correlations': {}, 'total_runtime': 0, 'last_cycle': 0}

    def save_state(self):
        print(f"\n💾 Saving state to {CONFIG.state_file}...")
        ...
        with open(CONFIG.state_file, 'wb') as f: pickle.dump(state, f)
        print("  ✅ State saved successfully.")
        
    def generate_data_chunk(self):
        start_n, end_n = self.current_n, self.current_n + CONFIG.data_chunk_size
        print(f"\nDATA GEN: Generating data for n = {start_n} to {end_n-1}...")
        ...
        with ProcessPoolExecutor(max_workers=CONFIG.n_workers) as executor:
            ...
            for i, future in enumerate(as_completed(futures)):
                if (i+1) % 250 == 0: print(f"  Processed {(i+1)} numbers in chunk...")
                res = future.result()
                if res and 'omega' in res: new_data.append(res)
        
        self.data.extend(new_data)
        self.current_n = end_n
        print(f"  DATA GEN: Added {len(new_data)} new data points. Total: {len(self.data)}")
    ...
    def run_discovery_engine(self):
        ...
        while not killer.kill_now:
            if not CONFIG.perpetual_mode and cycle >= CONFIG.max_cycles:
                print("🏁 Reached max cycles. Shutting down.")
                break
            
            cycle += 1
            print("\n" + "="*80); print(f"🚀 STARTING DISCOVERY CYCLE {cycle} | Current n = {self.current_n}"); print("="*80)
            ...
            if cycle % CONFIG.analysis_interval_cycles == 0:
                ...
                self.discoveries['best_nn'] = {'fitness': self.nas_searcher.best_fitness, 
    ...
    def periodic_report(self):
        ...
        if law and 'alpha' in law:
            print("\n  --- Sieve Echo Law ---")
            print(f"  - Empirical: H = {law['alpha']:.4f} * log(ω) + {law['beta']:.4f} (R² = {law['r_squared']:.3f})")
        ...
        print(f"\n  - Best Unbiased Formula (MSE): {unbiased_f.get('fitness', 'N/A'):.5f}")
        print(f"  - Best Theory-Guided Formula (MSE): {theory_f.get('fitness', 'N/A'):.5f}")
        print(f"  - Best NN Fitness (1/(1+MSE)): {nn.get('fitness', 'N/A'):.4f}")
        print("-" * 80)
    ...
    def final_report(self):
        ...
        if dead_ends:
            ...
            print(f"  - The following features showed low predictive power: {', '.join(dead_ends[:10])}...")
            
        print("\n  --- Best Discovered Formulas ---")
        unbiased_f = self.discoveries.get('unbiased_formula', {})
        theory_f = self.discoveries.get('theory_formula', {})
        print(f"  - Unbiased: {unbiased_f.get('formula', 'N/A')} (Fitness: {unbiased_f.get('fitness', 'N/A'):.4f})")
        print(f"  - Theory-Guided: {theory_f.get('formula', 'N/A')} (Fitness: {theory_f.get('fitness', 'N/A'):.4f})")
        
        print("\n✨ Discovery engine has shut down. All findings preserved in state file.")
...
def main():
    print("="*80)
    print("🔬 SIEVE ECHO GRAND UNIFIED ENGINE - V12.0")
    print("="*80)
