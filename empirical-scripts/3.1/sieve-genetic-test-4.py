...
class GPUPatternAnalyzer:
    ...
    def compute_repetend(self, n: int, base: int, max_length: int = 10000) -> List[int]:
        ...
        while remainder not in seen and len(digits) < max_length:
            seen[remainder], remainder = len(digits), (remainder * base) % n
            digits.append((remainder * base) // n) # Note: Original code had this line after, corrected order.
        result = digits[seen.get(remainder, 0):]
    ...
    def _lempel_ziv_complexity(self, p: List[int]) -> int: # Corrected & Efficient
        ...
        return c
    def _count_runs(self, p: List[int]) -> int: return 1 + sum(1 for i in range(1, len(p)) if p[i] != p[i-1]) if len(p)>1 else 1
...
class GeneticFeatureEvolver:
    ...
    def evolve(self, test_data: List[Tuple[int, Dict]], start_time: float):
        ...
        for gen in range(self.generation, self.config.generations):
            ...
            if gen % self.config.checkpoint_interval == 0 and gen > self.generation: self.save_checkpoint(gen)
    ...
    def save_checkpoint(self, generation: int):
        ...
        with open(filename, 'wb') as f:
            pickle.dump({
                'generation': generation, 'best_individual': self.best_individual,
                'best_fitness': self.best_fitness, 'fitness_history': self.fitness_history,
                'population': self.population, 'config': self.config,
...
def main():
    ...
    parser.add_argument('--hours', type=float, default=12.0, help='Runtime hours')
    ...
    if args.checkpoint:
        ...
        except Exception as e: logger.log(f"Failed to load checkpoint: {e}", "ERROR")
    
    try:
        explorer.run_complete_exploration()
    except KeyboardInterrupt:
        logger.log("\nExploration interrupted. Saving checkpoint and report.", "WARNING")
        if explorer.genetic_evolver.generation > 0: explorer.genetic_evolver.save_checkpoint(explorer.genetic_evolver.generation)
